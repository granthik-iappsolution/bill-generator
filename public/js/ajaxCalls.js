...
function ajaxCallFormSubmit(formElement, isToast, loadingMsg, data = {}, callBackFunction = undefined,
    ...
    $.ajax({
        ...
        contentType: false,
        beforeSend: function () {
            showWaitMeLoading(loadingMsg, '', waitMeContainer);
        },
        success: function (res) {
            let ajax_result = {status: 200, message: res.message, data: res.data}
            if(! res.hasOwnProperty('message')) {
                hideWaitMeLoading('', waitMeContainer);    return;
            }
            if (!isToast) {
            ...
            hideWaitMeLoading('', waitMeContainer);
        },
        error: function (jqXHR) {
            let errorRes = JSON.parse(jqXHR.responseText);
...
function ajax_loadAlertBox(ajax_Response, formElement) {
    ...
    if (ajax_Response.status === 422) {
        ...
        ajax_alert_div.append(titleMsg);
    ...
    ajax_alert_div.append('<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>');
